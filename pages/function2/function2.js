...
Page({
    data: {
        ...
        groupnum: 0, //自动生成的组数
        groupnum2: 0, //手动检测的组数
        boolnumgood: true, //显示好的手动检测内容
        boolnumbad: true,//显示不好的手动检测内容
        date:"", //这个是当前的日期
    },
    // 提前加载昵称和设置的内容
    onShow: function () {
    ...
    auto_exam() {
        ...
        // 计算组数
        if (this.data.hunum >= this.data.xinum) {
            this.setData({
                groupnum: this.data.hunum
            })
        } else {
            this.setData({
                groupnum: this.data.xinum
            })
        }
    ...
    self_exam() {
        wx.vibrateShort()
        if (this.data.groupnum == this.data.groupnum2) {
            this.setData({
                boolnumgood: false
            })
